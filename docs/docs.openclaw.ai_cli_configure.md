...
`agents.defaults.models` allowlist (what shows up in `/model` and the model picker).Tip: `openclaw config` without a subcommand opens the same wizard. Use
`openclaw config get|set|unset` for non-interactive edits.Related:
...
[Onboard](https://docs.openclaw.ai/cli/onboard) [Doctor](https://docs.openclaw.ai/cli/doctor)
